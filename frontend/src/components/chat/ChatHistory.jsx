...
import React, { useRef, useEffect } from 'react';
import MessageBubble from './MessageBubble'; // Corrected import extension if needed
import { motion, AnimatePresence } from 'framer-motion';
        ...
        <div ref={chatHistoryRef} className="flex-1 overflow-y-auto p-4 space-y-4">
            <AnimatePresence initial={false}>
                {messages.map((msg) => (
                    ...
                    >
                        <MessageBubble
                            id={msg.id} // CRITICAL: Pass the id here
                            sender={msg.sender}
                            text={msg.text}
                            thinking={msg.thinking}
                            references={msg.references}
            ...
            {isLoading && messages.length > 0 && (
                 <motion.div 
                    ...
                    className="flex justify-start pl-2 mt-2"
                 >
                    <div className="message-bubble bot-message bg-surface-light dark:bg-surface-dark p-2 inline-flex items-center gap-1 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
